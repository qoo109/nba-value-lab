...
| PR | 狀態 | 用途 |
|---|---|---|
| #67 — No-cost Timestamped Odds Metadata Census v1 | Draft / validated negative result | 凍結 8 個候選的授權、來源、bookmaker 與 timestamp 審查結果。 |
...
- 不把單一 game-level odds row、Opening／Closing label 或 first-seen 當成可靠 point-in-time snapshot。
...
- 不降低既有 30-game sample、coverage、PIT、identity、price 或 overround gates。
...
PR #67 metadata census Artifact：
