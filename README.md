...
目前已完成五季 Historical Gold、Logistic + Elo Walk-forward v2、Calibration Gate、Closing Market Benchmark、Market Residual Analysis、Rest／Travel holdout、官方傷病報告 importer、Player Identity、Expected Minutes research proxy、Team Injury Burden、Multi-report Injury Panel、Injury Residual Audit、2023-24 Historical Silver source archive reconciliation，以及 privacy-safe source-gap exception manifest。
...
- source-gap exception manifest 已以 aggregate-only 方式驗證；它不包含 game IDs、日期或隊伍代碼，也不授權 row patch、Silver／Gold rebuild 或模型使用。下一步僅是 QA／coverage integration policy design。
...
- `data/research/historical-silver-2023-24-source-gap-exception-current-status-v1.json`：validated manifest 與下一個 integration-policy 節點。
- `index.html`：網站結構。
...
Legacy UI 內的勝率與候選仍可能包含示範資料。正式研究管線已完成 Walk-forward、Calibration Gate、Closing Market Benchmark、多個 holdout／殘差實驗、source archive reconciliation 與 aggregate-only source-gap exception manifest，但尚未完成 real timestamped odds backfill、executable market backtest、CLV、EV、ROI、Drawdown 與正式鎖版前瞻測試。
