...
下一次繼續時，先檢查 GitHub `main`、Open PR、Actions 與 Artifact QA。若此 request design 尚未合併，先完成 validator 與 aggregate-only Artifact QA；不得直接跳到帳號建立、Key 連接或 network execution。
