...
old_facts = """bloombet probe execution count: 0 / 1
injury panel activation: 41 independent games / 31 T-60 selected / below 100-game gate"""
new_facts = """bloombet probe execution count: 0 / 1
...
injury panel activation: 41 independent games / 31 T-60 selected / below 100-game gate"""
if text.count(old_facts) != 1:
    raise SystemExit("unexpected PROJECT_STATUS fact anchor count")
text = text.replace(old_facts, new_facts, 1)

old_mainline = "BLOOMBET_FREE_API_ZERO_COST_SCHEMA_PROBE_AWAITING_EXPLICIT_USER_APPROVAL"
new_mainline = "DESIGN_SOURCE_AGNOSTIC_PRIVATE_FORWARD_ODDS_COLLECTOR_V1"
if text.count(old_mainline) < 1:
    raise SystemExit("missing BloomBet sub-mainline anchor")
text = text.replace(old_mainline, new_mainline, 1)

old_status = "BLOCKED — ACCOUNT CREATION AND PRIVATE API KEY CONNECTION NOT AUTHORIZED"
new_status = "READY — DESIGN ONLY / NO ACCOUNT OR API KEY REQUIRED"
if text.count(old_status) != 1:
    raise SystemExit("unexpected prerequisite status anchor count")
text = text.replace(old_status, new_status, 1)

evidence_anchor = "## Completed Evidence\n\n### BloomBet Free API public schema, terms and timestamp-semantics review"
...
if text.count(evidence_anchor) != 1:
    raise SystemExit("unexpected completed evidence anchor count")
