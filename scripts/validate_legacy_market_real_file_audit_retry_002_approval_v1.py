#!/usr/bin/env python3
"""Policy-only approval validator for Legacy Market Archive retry request 002."""
...
def validate_approval(
    ...
    check("request_previous", request.get("previous_request_id") == "LEGACY-MARKET-ARCHIVE-AUDIT-2026-07-21-001")
    check("request_previous_run", request.get("previous_run_id") == 29804975869)
    check("request_repair", request.get("repair_commit") == "613ce3a6232780c486d899b02dd7a99e799b0a27")
    check("request_entrypoint", request.get("repair_entrypoint") == "scripts/run_user_supplied_legacy_market_archive_real_file_audit_once_v1_1.py")
    ...
    check("request_not_approved", request.get("approval_granted") is False)
    check("request_disabled", request.get("execution_enabled") is False)
    check("request_count_zero", request.get("execution_count") == 0)
    for key in ("candidate_identity_changed", "reference_scope_changed", "scientific_contract_changed", "frozen_gates_changed", "output_boundary_changed", "raw_files_uploaded"):
        check(f"request_{key}", request.get(key) is False)
    check("request_raw_rows", request.get("raw_rows_emitted") == 0)
    check("request_stake", request.get("formal_stake") == 0)
    ...
    check("approval_response", approval.get("user_response") == EXPECTED_USER_RESPONSE)

    context = approval.get("context_binding", {})
    check("context_request", context.get("request_packet") == "data/research/legacy-market-real-file-audit-retry-request-002-v1.json")
    check("context_pr", context.get("request_packet_merge_pr") == 112)
    check("context_commit", context.get("request_packet_main_commit") == "3805bfb6ccc6fe13bd46c40e523eb225fd7ee506")
    check("context_validation_run", context.get("request_validation_workflow_run_id") == 29807027441)
    check("context_validation_job", context.get("request_validation_job_id") == 88559621609)
    check("context_repair_pr", context.get("repair_pr") == 111)
    check("context_repair_commit", context.get("repair_commit") == "613ce3a6232780c486d899b02dd7a99e799b0a27")

    auth = approval.get("execution_authorization", {})
    check("auth_one_time", auth.get("one_time_only") is True)
    check("auth_max_one", auth.get("maximum_execution_count") == 1)
    check("auth_prior_zero", auth.get("executions_recorded_before_approval") == 0)
    check("auth_dispatch", auth.get("workflow_dispatch_only") is True)
    check("auth_main", auth.get("approved_ref") == "refs/heads/main")
    check("auth_dataset", auth.get("dataset_handle") == EXPECTED_DATASET)
    check("auth_file", auth.get("exact_candidate_file") == EXPECTED_FILE)
    check("auth_bytes", auth.get("candidate_file_bytes") == EXPECTED_BYTES)
    check("auth_sha", auth.get("candidate_file_sha256") == EXPECTED_SHA256)
    check("auth_years", auth.get("reference_season_start_years") == EXPECTED_YEARS)
    check("auth_network", auth.get("temporary_network_download_allowed") is True)
    check("auth_candidate_read", auth.get("temporary_candidate_rows_may_be_read") is True)
    check("auth_reference_read", auth.get("temporary_reference_rows_may_be_read") is True)
    check("auth_rebuild", auth.get("temporary_reference_rebuild_allowed") is True)
    check("auth_aggregate", auth.get("aggregate_outputs_only") is True)
    check("auth_one_output", auth.get("maximum_public_output_files") == 1)
    check("auth_size", auth.get("maximum_public_output_bytes") == 1048576)
    check("auth_enabled", auth.get("execution_enabled_for_this_request") is True)

    ack = approval.get("acknowledgements", {})
    false_keys = (
        "candidate_identity_changed", "reference_scope_changed", "scientific_contract_changed",
        "frozen_gates_changed", "output_boundary_changed",
        "raw_rows_or_raw_files_may_be_uploaded_as_artifact", "candidate_csv_may_be_uploaded_as_artifact",
        "historical_silver_database_may_be_uploaded_as_artifact", "historical_gold_database_may_be_uploaded_as_artifact",
        "source_archives_may_be_uploaded_as_artifact", "unmatched_keys_or_game_ids_may_be_emitted",
        "opening_or_closing_labels_authorized", "point_in_time_market_backtest_authorized",
        "clv_ev_roi_drawdown_authorized", "historical_silver_replacement_authorized",
        "historical_gold_replacement_authorized", "model_training_or_retraining_authorized",
        "betting_edge_claim_authorized", "betting_decision_layer_authorized",
    )
    for key in false_keys:
        check(f"ack_{key}", ack.get(key) is False)
    check("ack_stake", ack.get("formal_stake") == 0)

    next_state = approval.get("next_state_if_approval_validation_passes", {})
    check("next_state", next_state.get("formal_state") == READY)
    check("next_once", next_state.get("ready_for_one_time_aggregate_execution") is True)
    check("next_no_repeat", next_state.get("ready_for_repeat_execution") is False)
    check("next_no_market", next_state.get("ready_for_point_in_time_market_backtest") is False)
    ...
    check("impl_source", implementation.get("source_id") == SOURCE_ID)
    check("impl_file", candidate.get("file_name") == EXPECTED_FILE)
    check("impl_bytes", candidate.get("file_bytes") == EXPECTED_BYTES)
    check("impl_sha", candidate.get("file_sha256") == EXPECTED_SHA256)
    check("impl_no_derived", candidate.get("cleaned_or_derived_file_substitution_allowed") is False)
    check("impl_not_executed", implementation.get("validation_mode", {}).get("real_file_audit_executed") is False)
    check("impl_stake", implementation.get("downstream_permissions", {}).get("formal_stake") == 0)
    ...
    check("current_retry_unapproved", retry.get("approval_granted") is False)
    check("current_retry_disabled", retry.get("execution_enabled") is False)
    check("current_retry_zero", retry.get("execution_count") == 0)
    check("current_not_executed", current.get("real_cross_source_audit_executed") is False)
    check("current_stake", current.get("formal_stake") == 0)
...
def self_test(request, approval, implementation, current) -> dict[str, bool]:
    baseline = validate_approval(request, approval, implementation, current, REQUEST_ID, "workflow_dispatch", "refs/heads/main")
    ...
    for name, (kind, value) in mutations.items():
        ...
        report = validate_approval(request, app, implementation, current, confirmation, event, ref)
...
def main() -> int:
    ...
    print(json.dumps({"formal_state": report["formal_state"], "checks_passed": report["checks_passed"], "checks_total": report["checks_total"], "formal_stake": report["formal_stake"]}, ensure_ascii=False, indent=2))
